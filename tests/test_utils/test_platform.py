...
class TestGetDownloadURL:
    ...
    def test_get_download_url_unsupported_platform(self, mock_platform_info):
        ...
        with pytest.raises(RuntimeError, match="Platform windows/x64 is not supported for ffmpeg"):
    ...
    def test_get_download_url_unknown_platform(self, mock_platform_info):
        ...
        mock_platform_info.return_value = (OperatingSystem.UNKNOWN, Architecture.UNKNOWN)
        
        with pytest.raises(RuntimeError, match="Platform unknown/unknown is not supported for yt-dlp"):
...
class TestPlatformLogging:
    ...
    def test_detect_os_logs_warning_for_unknown_os(self, mock_system, caplog):
        ...
        assert any("Unknown operating system detected: freebsd" in msg for msg in warning_messages)
    ...
    @patch("src.utils.platform.platform.machine")
    def test_detect_architecture_logs_warning_for_unknown_arch(self, mock_machine, caplog):
        ...
        assert any("Unknown architecture detected: sparc" in msg for msg in warning_messages)